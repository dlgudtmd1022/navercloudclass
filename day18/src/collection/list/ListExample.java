...
public class ListExample {
	...
	public static void main(String[] args) {
		...
		// 제네리 : 타입 안정화를 위해 객체 내부에 저장되는 타입을 일치시키는 방법
		// String만 저장할 수 있는 컬렉션
		List<String> list = new ArrayList<>(); // 1.8부터는 생성자에 제네릭 표기 안해도됩니다.
//		List<String> list = new LinkedList<>(); // 1.8부터는 생성자에 제네릭 표기 안해도됩니다.
		
		String str1 = "Java";
		String str2 = "SptringBoot";
		System.out.println(list);// 컬랙션 특성상 toString()이 이미 재정의되어 주속 안나오고 내부요소가 나옴
		...
		// list에 객체 삽입 add(index, 객체) : 데이터를 해다 인덱스에 삽임
		list.add(2, "Network");
		System.out.println(list);
...
//		list[3] = "PostgreSQL"; // 일반 수정 구문 사용 불가
		...
		System.out.println("-----------------------------");
		// 향상된 for문도 사용 가능(List<Stirng>으로 선언했으므로 String으로 받기)
		for(String str : list) {
		...
		list.clear();
		System.out.println(list); // list라는 박스는 남아있고 내부의 자료만 삭제됨
